...
const SyncMSVG:React.FC<MSVG> = ({ focus}) => {
  ...
  return (
    ...
    >
        <svg width="100%" height="16100^" viewBox="0 0 166 166" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <feOffset/>
            <motion.feGaussianBlur ref={blurScope} initial={focus?{stdDeviation:"0"}:{stdDeviation:"8.9022"}} stdDeviation="8.9022"/>
            ...
            <radialGradient id="paint0_radial_50_303" cx="0" cy="0" r="1" gradientUnits="userSpaceOnUse" gradientTransform="translate(83 83) rotate(89.5001) scale(64.5409)">
                <motion.stop ref={stopOne} initial={focus?{stopColor:styleSheet.gradients.sync.off}:{stopColor:styleSheet.gradients.sync.on.stopOne}} offset="0.310127"/>
                <motion.stop ref={stopTwo} initial={focus?{stopColor:styleSheet.gradients.sync.off}:{stopColor:styleSheet.gradients.sync.on.stopTwo}} offset="0.554923"/>
                <motion.stop ref={stopThree} initial={focus?{stopColor:styleSheet.gradients.sync.off}:{stopColor:styleSheet.gradients.sync.on.stopThree}} offset="0.799719"/>
                <motion.stop ref={stopFour} initial={focus?{stopColor:styleSheet.gradients.sync.off}:{stopColor:styleSheet.gradients.sync.on.stopFour}} offset="1"/>
            </radialGradient>
            </defs>
        </svg>
    </motion.div>
